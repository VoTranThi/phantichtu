function resize() {
    ...
    heatmap.attr("transform", "translate(" + (width - 20.5) + ",.5)").select("rect.frame").attr("height", height - 1),
...
function processText(e) {
    var t, n = 0, r = 0, i = 0, o = 0, a = text.append("span").text("m"), c = 285 / a.node().offsetWidth;
    a.remove(),
    tokens = [],
    lines = [];
    for (var i = []; t = re.exec(e); ) {
        ...
        /\r\n\r\n|\r\r|\n\n/.test(l) ? (lines.push(i, []),
        i = [],
        o = t[0].length) : (o += t[0].length + !!l.length,
        o > c && (o = t[0].length,
        lines.push(i),
        i = []));
        var s = {
...
function getURL(e, t) {
    ...
    if ("https:" === location.protocol && /^http:/.test(e))
        proxy(e, n);
    else
        try {
...
function change() {
    if (!location.search)
        return void showHelp();
    var e = state ? state.source : null;
    if (state = urlParams(location.search.substr(1)),
    state.source !== e && state.source)
        ...
        keyword.property("value", t).node().select(),
        t = t.toLowerCase().match(re),
        treeG.call(tree.sort("occurrence" === state.sort ? function(e, t) {
            return e.index - t.index
        }
        : function(e, t) {
            return t.count - e.count || e.index - t.index
        }
        ).reverse(+state.reverse).phraseLine(+state["phrase-line"]).prefix(t)),
...
function currentLine(e) {
    if (!e)
        return 0;
    for (var t = e.children; t && t.length; )
        e = t[0],
        t = e.children;
...
function clearHighlight() {
    for (var e = -1; ++e < tokens.length; )
        tokens[e].highlight = !1
}
...
function updateHeatmap() {
    var e = heatmap.selectAll("line").data(selectedLines)
      , t = textViewer.size();
    e.enter().insert("line", "rect").attr("x2", 20),
    e.attr("transform", function(e) {
        return "translate(0," + height * e / t + ")"
    }),
    e.exit().remove();
    var n = page.datum();
    n.h = Math.min(height, Math.max(10, height * height / (textViewer.rowHeight() * lines.length))),
...
function drag(e) {
    ...
    text.node().scrollTop = e.y * textViewer.rowHeight() * lines.length / height,
    page.attr("y", e.y)
}
function hoverKey() {
        ...
        function e(e) {
            function o(o) {
                e.each(function() {
                    ...
                    s.enter().append("div").attr("class", "row"),
                    s.exit().remove(),
                    s.order().call(t)
                })
            }
            ...
            a.enter().append("div").attr("class", "current"),
            e.selectAll("div.after").data([0]).enter().append("div").attr("class", "after"),
            e.on("scroll.longscroll", function() {
                r = Math.floor(this.scrollTop / i),
                o(this.scrollTop)
            }),
            o(0),
            e.each(function() {
...
function(e) {
    function t() {
        function e(a) {
            ...
            if (s) {
                ...
                i(h),
                u.transition().duration(U).attr("x", function(e) {
                    return d(e.x)
                }).attr("y", function(e) {
                    return e.y
                });
                var p = u.enter().append("text")
                  , g = u.selectAll("tspan").data(function(e) {
                    var t = e.tokens;
                    return t.slice(0, 50).map(function(t) {
                        return {
                            data: e,
                            token: t
                        }
                ...
                u.attr("text-anchor", L ? "end" : null).transition().delay(U).attr("visibility", null),
                u.exit().remove();
                var v = a.selectAll("path.link").data(j.links(h), function(e) {
                    return c(e.source) + ":" + c(e.target)
                });
                v.attr("visibility", null).transition().duration(U).attr("d", I),
                v.enter().insert("path", "a").attr("class", "link").attr("d", I).attr("visibility", "hidden").transition().delay(U).attr("visibility", null),
                v.exit().remove(),
                U = 500,
                n(b)
            }
        ...
        function a(e, t, n) {
            ...
            return p(c),
            u(c, C - 1),
            n && n.length && (n = a[n.join(" ")],
            n && o(n.node || n)),
            c
        }
        function u(e, t) {
            if (0 >= t)
                return void (e.children = []);
            var n = e.count * A;
            e.children = e.children.filter(function(e) {
                return e.count > n ? (u(e, t - 1),
                !0) : void 0
            })
        }
        function d(e) {
        ...
        function p(e, t) {
            ...
            if (r) {
                if (1 === r) {
                    var i = e.children[0];
                    return (L ? e.tokens.unshift : e.tokens.push).apply(e.tokens, i.tokens),
        ...
        function f(e) {
            var t = [];
            do
                (L ? t.push : t.unshift).call(t, d3.merge(e.tokens.map(function(e) {
                    return [e.whitespace ? " " : "", e.token]
                })));
            while (e = e.parent);return t[0] = t[0].slice(1),
            d3.merge(t).join("")
        }
        function g(e) {
            if (L)
                for (var t = e.tokens[0].index, n = t - H; --t >= 0 && !v(_[t + 1]) && (t >= n || !x(_[t])); )
                    e.tokens.unshift(_[t]);
            else
                for (var t = e.tokens[e.tokens.length - 1].index, n = t + H; ++t < _.length && !v(_[t]) && (n > t || !x(_[t - 1])); )
                    e.tokens.push(_[t])
        }
        function v(e) {
            return z && /[\r\n]/.test(e.whitespace)
        ...
        function m(e, t) {
            e.count * t < 5 ? e.children && e.children.length > 1 && (e._collapsed = e.children,
            e.children = [e.children[0]]) : e._collapsed && !e._children && (e.children = e._collapsed),
            e.children && e.children.forEach(function(e) {
                m(e, t)
            })
        }
        var y, k, w, b, T = 5, C = 10, H = 15, A = .01, L = !1, M = s, _ = [], R = [], z = !1, E = d3.dispatch("prefix"), S = d3.scale.sqrt().range([0, 25]), j = d3.layout.partition().sort(h).value(function(e) {
            return e.count
        ...
        return e.tokens = function(t) {
            ...
            _ = t,
            w = {};
            for (var n, r = 0, i = _.length; i > r; ++r)
                n = M.call(e, _[r], r),
        ...
        ,
        e.prefix = function(t) {
            return arguments.length ? (R = t.slice(),
        ...
        ,
        e.width = function(t) {
            return arguments.length ? (j.size([j.size()[0], y = +t]),
            e) : y
        }
        ,
        e.height = function(t) {
            return arguments.length ? (j.size([k = +t, j.size()[1]]),
            e) : k
        }
        ,
    ...
    function o(e) {
        for (a(e); e.parent; )
            e.parent._children || (e.parent._children = e.parent.children,
            e.parent.children = [e],
    ...
    function a(e) {
        e._children && (e.children = e._children,
        delete e._children,
        e.count = e._count,
        delete e._count),
    ...
    function c(e) {
        var t = [];
        do
            t.unshift(e.index + "-" + e.tokens.length),
            e = e.parent;
        while (e);return t.join("/")
    }
    function l(e) {
...
var width, height, tree = wordtree().on("prefix", function(e) {
    ...
    t.enter().append("a").attr("href", function(e) {
        ...
        return e.whitespace && this.parentNode.insertBefore(document.createTextNode(" "), this),
        e.token
    }),
...
var div = d3.select("#examples").selectAll("div.thumb").data([{
    ...
    var t = [];
    for (var n in e)
        "name" !== n && t.push(encodeURIComponent(n) + "=" + encodeURIComponent(e[n]));
    e.url = "./?" + t.join("&")
});
...
var re = new RegExp("[" + unicodePunctuationRe + "]|\\d+|[^\\d" + unicodePunctuationRe + "0000-001F007F-009F002000A01680180E2000-200A20282029202F205F3000".replace(/\w{4}/g, "\\u$&") + "]+","g"), vis = d3.select("#vis"), svg = vis.append("svg"), clip = svg.append("defs").append("clipPath").attr("id", "clip").append("rect"), treeG = svg.append("g").attr("transform", "translate(0,20)").attr("clip-path", "url(#clip)"), heatmap = svg.append("g"), lines = [], text = d3.select("#text").on("scroll", scroll), hits = d3.select("#hits"), keyword = d3.select("#keyword"), source = d3.select("#source"), state = {}, tokens, selectedLines = [];
...
d3.select(window).on("keydown.hover", hoverKey).on("keyup.hover", hoverKey).on("resize", resize).on("popstate", change),
change(),
resize(),
...
d3.select("#sort").selectAll("option").data(["frequency", "occurrence"]).enter().append("option").attr("value", String).text(String),
d3.select("#reverse").property("checked", +state.reverse).on("change", function() {
    url({
